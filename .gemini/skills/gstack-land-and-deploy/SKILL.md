...
to guide deploy verification. If nothing is detected, ask the user via ask the user
...
Present the full dry-run results to the user via ask the user:
...
4. If no deploy workflows detected and no URL provided: use ask the user once:
...
If deploy fails (`conclusion` is `failure`): use ask the user:
...
If any fail: show the evidence (screenshot path, console errors, perf numbers). Use ask the user:
