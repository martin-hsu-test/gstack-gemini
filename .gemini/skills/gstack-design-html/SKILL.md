...
Then check for prior finalized.html. If `FINALIZED` was also found, use ask the user:
...
3. If in plan-driven or freeform mode (no approved PNG), design from context:
   ...
   - **Freeform:** use ask the user to gather what the user wants to build. Ask about:
...
If a framework is detected, use ask the user:
...
Maximum 10 iterations. If the user hasn't said "done" after 10, use ask the user:
