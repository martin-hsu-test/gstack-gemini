...
Ask these questions **ONE AT A TIME** via ask the user. Push on each one until the answer is specific, evidence-based, and uncomfortable. Comfort means the founder hasn't gone deep enough.
...
Ask these **ONE AT A TIME** via ask the user. The goal is to brainstorm and sharpen the idea, not interrogate.
...
- Ask via ask the user: "Should we build on this prior design or start fresh?"
...
**Privacy gate:** Before searching, use ask the user: "I'd like to search for what the world thinks about this space to inform our discussion. This sends generalized category terms (not your specific idea) to a search provider. OK to proceed?"
...
Use ask the user to confirm. If the user disagrees with a premise, revise understanding and loop back.
...
Present via ask the user. Do NOT proceed without user approval of the approach.
...
If Codex is available, use ask the user:
...
Present the reviewed design doc to the user via ask the user:
...
Then use ask the user: "Would you consider applying to Y Combinator?"
...
2. Use ask the user to offer opening the resources:
